...
│                    用户                           │
├──────────────────────────────────────────────────┤
│  GUI (gui.py)          CLI (sftp_transfer.py)    │
│  tkinter 界面           --host/--user/--password  │
│        │                      │                   │
│        ▼                      ▼                   │
│  ┌─────────────┐    ┌──────────────────┐         │
│  │ SSHManager  │    │   paramiko.SSH   │         │
│  │  · connect  │    │   · connect      │         │
│  │  · sftp     │    │   · open_sftp    │         │
│  │  · tunnel   │    └──────────────────┘         │
│  └─────────────┘              │                   │
...
| 通道 | 传输方式 | 优点 | 缺点 |
|------|---------|------|------|
| **SFTP** | paramiko SFTP 直连 | 无需额外进程，GUI 默认使用 | 不支持断点续传 |
...
| `_History` | 连接历史缓存（IP / 端口 / 用户名） |
| `_RemoteBrowser` | 远程文件浏览对话框 |
| `_LocalBrowser` | 本地文件浏览对话框 |
...
- 最多保留 10 条（`_MAX_HISTORY`）
- 新纪录写入时先去重再置顶（最近使用的排最前）
- `_History.record()` 在 SSH 连接成功后调用
- `_History.last()` 在应用启动时预填输入框

**实现位置**：`gui.py` `_History` 类（约 40 行）。
...
SFTP 模式更简洁（无需部署额外进程），适合 GUI 交互场景。HTTP 模式更灵活（支持断点续传、客户端零依赖），适合脚本和大文件场景。
